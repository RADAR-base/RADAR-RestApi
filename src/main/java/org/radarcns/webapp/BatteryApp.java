...
public class BatteryApp {
    ...
    @ApiOperation(
            ...
            notes = "Return the last seen Battery level value of type stat for the given userID and sourceID")
    @ApiResponses(value = {
            @ApiResponse(code = 204, message = "No value for the given parameters, in the body" +
                    "there is a message.avsc object with more details"),
            @ApiResponse(code = 200, message = "Return a dataset.avsc object containing last seen battery.avsc value for the required statistic function")})
    public Response getRealTimeUser(
        ...
        try {
            ...
            Dataset battery = BatteryDAO.getInstance().valueRTByUserSource(userID, sourceID, Unit.percentage, stat, context);

            if (battery.getDataset().isEmpty()) {
                logger.info("No data for the user {} with source {}", userID, sourceID);
            }

            return ResponseHandler.getJsonResponse(request, battery, sensorName);
        }
        catch (Exception e){
    ...
    @ApiResponses(value = {
            @ApiResponse(code = 204, message = "No value for the given parameters, in the body" +
                    "there is a message.avsc object with more details"),
            @ApiResponse(code = 200, message = "Return a dataset.avsc object containing all available battery.avsc values for the required statistic function")})
    public Response getAllByUser(
        ...
        try {
            ...
            Dataset battery = BatteryDAO.getInstance().valueByUserSource(userID, sourceID, Unit.percentage, stat, context);
    ...
    @ApiResponses(value = {
            @ApiResponse(code = 204, message = "No value for the given parameters, in the body" +
                    "there is a message.avsc object with more details"),
            @ApiResponse(code = 200, message = "Return a dataset.avsc object containing all battery.avsc values belonging to the time window [start - end] for the required statistic function")})
    public Response getByUserForWindow(
        ...
        try {
            ...
            Dataset battery = BatteryDAO.getInstance().valueByUserSourceWindow(userID, sourceID, Unit.percentage, stat, start, end, context);
